...
- **데이터셋 저장/불러오기**: 세션 기반 지속적 데이터 관리 (브라우저 세션 간 유지)
...
- **데이터셋 비교**: 여러 데이터셋 간 통계 비교 분석 (결과는 데이터셋 관리 섹션 바로 아래 표시)
...
- **🔍 이상치 결과 다운로드**: 이상치 제거 후 분석 결과 CSV
- **🆕 통합 다운로드**: 원본 데이터 + 이상치 판정 결과 통합 CSV
...
### 4. 데이터셋 관리
#### 💾 **저장 및 불러오기**
- **데이터셋 저장**: 입력 필드에 이름을 지정하여 현재 작업 저장
- **데이터셋 불러오기**: 드롭다운에서 저장된 데이터셋 선택 후 불러오기
- **데이터셋 삭제**: 확인 대화상자 후 선택된 데이터셋 안전하게 삭제
- **저장된 개수 표시**: 현재 저장된 데이터셋 개수 실시간 표시
- **자동 지속성**: Flask-Session 기반으로 브라우저 세션 간 데이터 유지
- **안정적 저장**: 페이지 새로고침이나 브라우저 재시작 후에도 저장된 데이터 유지

#### 📊 **데이터셋 비교**
- **다중 선택**: 체크박스 모달로 여러 데이터셋 선택 (최소 2개)
- **시각화 비교**: 선택된 데이터셋들을 하나의 차트로 통합 표시
- **통계 요약**: 각 데이터셋의 평균, 표준편차 등 비교 테이블
- **인터랙티브 차트**: Plotly 기반 확대/축소 및 데이터 확인 가능
- **결과 위치**: 데이터셋 관리 섹션 바로 아래에 비교 결과 표시
- **자동 스크롤**: 비교 결과 생성 시 해당 위치로 자동 스크롤

### 5. 실험군/대조군 데이터 관리
1. **실험군 데이터 입력**:
   - **샘플명**: 실험 샘플의 고유 식별자
   - **생산일자**: 실험 데이터의 생산 날짜 (날짜별 시각화 그룹핑)
   - **이상치 제거법**: 사용자가 직접 선택 (Manual, Z-Score, IQR, MAD)
   - **Size(nm) 평균**: 입자 크기 평균값
   - **PI 평균**: 다분산지수 평균값
   - **사용자 정의 필드**: 점도, 온도, 압력 등 다양한 측정 데이터

2. **대조군 데이터 입력** (레퍼런스):
   - **샘플명**: 대조군 샘플의 고유 식별자
   - **Size(nm) 평균**: 기준 입자 크기 평균값
   - **PI 평균**: 기준 다분산지수 평균값
   - **사용자 정의 필드**: 기준 측정 데이터
   - **생산일자 불필요**: 대조군은 레퍼런스이므로 생산일자 입력 없음

3. **이상치 제거법 정확도 향상**:
   - **명시적 선택**: 사용자가 직접 이상치 제거법을 선택하여 정보 정확도 향상
   - **자동 감지 보조**: Manual 선택 시에만 계산 결과와 매칭하여 자동 감지
   - **중복 방지**: Z-Score와 IQR 결과가 동일해도 사용자 선택 방법 우선 적용
   - **임계값 매칭**: 선택한 방법의 계산 결과와 일치하는 경우 임계값 정보 자동 추가
...
5. **샘플별 트렌드 분석**:
   - **생산일자별 그룹핑**: 실험군 데이터를 생산일자별로 정렬 및 시각화
   - **샘플별 트렌드**: Size/PI 값의 샘플별 변화 추이 시각화
   - **상관관계 분석**: Size-PI 간 상관계수 계산 및 시각화
   - **통계 요약**: 평균, 표준편차, 변동계수 등 종합 통계
   - **실험군/대조군 구분**: 실험군(파란색), 대조군(Reference Values, 초록색)

6. **사용자 정의 필드 상관관계 분석**:
   - **조건**: 사용자 정의 데이터 2개 이상 필요
   - **날짜별 시각화**: 실험군 데이터를 생산일자별로 색상 구분하여 표시
   - **대조군 구분**: 대조군 데이터는 'Reference Values (Control)'로 다이아몬드 마커 표시
   - **기준값 비교**: 필드명에 "점도" 또는 "viscosity" 포함 시 UHV, HV, LV 기준값과 비교
   - **상관계수**: Size-사용자 정의 필드 간 상관관계 수치 제공
   - **범례 최적화**: "Aa" 텍스트 제거로 깔끔한 범례 표시
...
#### 💾 **결과 다운로드**
- **🔍 이상치 결과 다운로드**: 이상치 제거 후 정제된 데이터만 포함
  - 샘플 정보 (샘플명, 생산일자, 패스 수)
  - 원본 데이터 및 통계
  - 각 방법별 필터링 결과 및 통계
  - 이상치 제거 정보
- **🆕 데이터+이상치 결과 다운로드**: 원본 데이터와 이상치 판정 결과 통합
  - 모든 원본 데이터 포함
  - 각 행별 Z-Score, IQR, MAD 이상치 여부 표시 (예/아니오)
  - 이상치 분석 요약 및 통계
  - 컬럼 설명 포함
...
### 🧪 **고급 분석 기능**
- **샘플별 트렌드 분석**: 실험군/대조군 샘플별 품질 변화 추적 (생산일자별 정렬)
- **실험군/대조군 비교 분석**: 실험 데이터와 레퍼런스 데이터 비교
- **생산일자별 그룹핑**: 실험군 데이터를 생산일자별로 색상 구분하여 시각화
- **사용자 정의 필드 상관관계 분석**: 입자 크기와 사용자 정의 변수 간 상관관계 시각화
- **그룹별 시각화**: 실험군(원형, 날짜별 색상)과 대조군(다이아몬드, 녹색) 구분 표시
- **이상치 제거법 정확도**: 사용자 선택 우선, 자동 감지 보조로 정보 정확성 향상
- **유연한 필드 관리**: 점도, 온도, 압력 등 다양한 측정 항목 지원
- **점도 호환 모드**: 점도 데이터 시 UHV, HV, LV 기준값 대비 생산 데이터 위치 분석
- **다중 인자 통계**: 상관계수, 트렌드, 변동계수 종합 분석
- **범례 최적화**: 모든 차트에서 "Aa" 텍스트 제거로 깔끔한 시각화
...
- **최종 업데이트**: 2025-07-17
- **주요 업데이트**:
  - **🚀 실험군/대조군 관리 시스템 전면 개선 (v3.0.0)**:
    - **실험군/대조군 독립 관리**: 패스 기반에서 샘플 기반으로 전환, 실험군과 대조군 별도 입력 폼
    - **생산일자 기반 시각화**: 실험군 데이터에 생산일자 필드 추가, 날짜별 색상 구분 시각화
    - **이상치 제거법 정확도 향상**: 사용자 직접 선택 방식으로 Z-Score/IQR 중복 매칭 문제 해결
    - **대조군 레퍼런스 최적화**: 대조군을 레퍼런스 데이터로 재정의, 생산일자 불필요
    - **샘플별 트렌드 분석**: "패스별"에서 "샘플별" 분석으로 명칭 변경, 생산일자 기반 정렬
    - **시각화 개선**: 대조군 범례를 'Reference Values'로 통일, "Aa" 텍스트 제거
    - **사용자 경험 향상**: 실험군 7컬럼(이상치 제거법 포함), 대조군 5컬럼 구조로 최적화
  - **🔧 세션 지속성 및 UI 최적화 (v2.9.0)**:
    - **Flask-Session 도입**: 브라우저 세션 간 데이터셋 지속성 보장
    - **비교 결과 위치 개선**: 데이터셋 비교 결과를 데이터셋 관리 섹션 바로 아래 표시
    - **코드 최적화**: 디버깅 코드 제거 및 성능 향상
    - **문서 업데이트**: 도움말 모달 및 README 최신화
  - **🔧 데이터셋 관리 개선 및 UI 최적화 (v2.8.0)**:
    - **데이터셋 저장 개선**: prompt 방식에서 입력 필드 방식으로 변경하여 사용자 경험 향상
    - **데이터셋 삭제 기능**: 확인 대화상자와 함께 안전한 삭제 기능 추가
    - **데이터셋 비교 기능**: 체크박스 모달로 다중 선택 및 시각화 비교 구현
    - **섹션 순서 최적화**: 워크플로우에 맞게 "이상치 검출 → 데이터셋 관리" 순서로 재배치
    - **전역 함수 등록**: 모든 데이터셋 관리 함수들의 전역 접근성 보장
    - **오류 처리 강화**: 각 기능별 적절한 오류 메시지 및 사용자 안내 추가
  - **🆕 통합 다운로드 기능 (v2.7.0)**:
    - **데이터+이상치 결과 다운로드**: 원본 데이터와 이상치 판정 결과를 하나의 파일로 통합
    - **각 행별 이상치 표시**: Z-Score, IQR, MAD 각 방법별 이상치 여부를 "예/아니오"로 표시
    - **통합 분석 보고서**: 원본 데이터, 이상치 분석 요약, 컬럼 설명이 포함된 종합 CSV
    - **사용자 친화적 UI**: 계산 완료 후 기존 다운로드 버튼과 함께 표시
    - **완벽한 메타데이터**: 샘플명, 생산일자, 분석일시 등 모든 메타데이터 포함
