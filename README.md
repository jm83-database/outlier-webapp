...
### 💾 **데이터 다운로드 및 업로드**
- **테이블 다운로드**: 입력한 원시 데이터를 CSV로 다운로드
  - **데이터만 다운로드**: 순수 데이터, 재업로드 호환 가능
  - **메타데이터 포함**: 샘플명, 생산일자 등 정보 포함
- **결과 다운로드**: 이상치 제거 후 분석 결과 CSV
- **스마트 업로드**: 메타데이터 포함 파일도 자동 파싱하여 로드
- **완벽한 사이클**: 다운로드 → 편집 → 업로드 워크플로우 지원
...
#### 📤 **테이블 다운로드**
- **다운로드 옵션 선택**: 버튼 클릭 시 옵션 모달 표시
- **데이터만 다운로드**: Excel에서 편집 후 재업로드 가능한 순수 데이터
- **메타데이터 포함**: 샘플 정보, 날짜 등이 포함된 완전한 기록
